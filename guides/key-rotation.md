# 密钥轮转
UKMS提供了密钥轮转功能，您可以通过定期轮转来加强密钥使用的安全性，有效地提升业务数据的安全性。
...
密钥支持多个密钥版本，同一个密钥下的多个密钥版本在密码学上互不相关，UKMS通过生成一个新的密钥版本来实现密钥的轮转，并使用最新的密钥版本进行密码运算操作，不支持指定某一个密钥版本。调用方无需修改任何代码：
- 轮转后系统生成新的底层密钥材料，KeyId 保持不变
- 新的加密操作使用新版密钥材料
- 旧版密钥材料仍被保留，用于解密之前加密的数据
- `LastRotationDate` 字段记录最近一次轮转时间

**重要**

-   密钥轮转仅新增密钥版本，密钥ID、别名等属性不会发生改变。
    
-   UKMS不会删除任何密钥版本，密钥版本仅随密钥删除而删除。
